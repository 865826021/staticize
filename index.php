...
if (is_file('./html/cms.html') && time()-filemtime('./html/cms.html') < 50 ) {
	require_once('./html/cms.html');
}
else	//不存在静态文件或距离上次修改的时间大于1分钟，则重新生成
{
	...
	//引入模板文件
	ob_start();
	require_once('./template/cms.php');
	//使用file_put_contents输出内容，生成纯静态页面
	file_put_contents("./html/cms.html", ob_get_contents());
